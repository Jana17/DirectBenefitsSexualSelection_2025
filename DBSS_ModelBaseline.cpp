...
struct Param {
    ...
    size_t used_seed;
    const size_t start_seed = 7;
    const size_t end_seed = 8; 

    const double initial_tau = 0.0;
    double initial_p = 0.0; 
    double initial_chi = 10; 
    ...
    double resource_min = 0.5;
    double resource_max = 1.5;
...
void find_mate(size_t& mother_id,
    ...
    rnd_j& rng,
    std::ofstream& ofs1,
    double& avg_survival_prob_rando,
    double& avg_survival_prob_rando_counter,
    int current_time) {

    mother_id = rng.random_number(pop_females.size());
    has_mated = false;
    ...
    while (true) {
        ...
        pop_males[father_id].NrRejections = pop_males[father_id].NrRejections + 1;
        if (rng.bernouilli(prob_mating)) {
            has_mated = true;
            pop_males[father_id].HasMated = 0;
            pop_females[mother_id].HasMated = 0;
            pop_males[father_id].NrRejections = pop_males[father_id].NrRejections - 1;
            pop_males[father_id].NrMatings = pop_males[father_id].NrMatings + 1;
            pop_females[mother_id].NrMatings = pop_females[mother_id].NrMatings + 1;
            break;
        }
    ...
    double offspring_survival_prob_rando = 1.0 - exp(-parameters.survival_scaling * pop_males[rando_id].gamma);
    double offspring_survival_prob_actualMate = 1.0 - exp(-parameters.survival_scaling * pop_males[father_id].gamma);
    avg_survival_prob_rando = avg_survival_prob_rando + offspring_survival_prob_rando;
    avg_survival_prob_rando_counter = avg_survival_prob_rando_counter + 1;

    //The following if statement outputs data. This data output should be used if data on the result of matings is wanted, else the if-statement should be commented out.
    /*if (current_time % parameters.save_interval == 0 && time != 0) {
        ofs1 << current_time << "," << parameters.used_seed << ","
            << pop_females[mother_id].p << "," << pop_females[mother_id].chi << ","
            << pop_males[father_id].tau << "," << pop_males[father_id].t << ","
            << pop_males[father_id].gamma << "," << pop_males[father_id].Res << ","
            << offspring_survival_prob_actualMate << "," << offspring_survival_prob_rando
            << '\n';
    }*/
}
...
void reproduce(const size_t& mother_id,
    ...
    double& avg_survival_prob_true,
    double& avg_survival_prob_true_counter) {

    double res_off = pop_males[father_id].gamma; 
    double offspring_survival_prob = 1.0 - exp(-parameters.survival_scaling * res_off);

    avg_survival_prob_true = avg_survival_prob_true + offspring_survival_prob;
    avg_survival_prob_true_counter = avg_survival_prob_true_counter + 1.0;
...
int main() {
    ...
    //This outputs data on population averages. If you want data on individuals or the result of matings instead, then this can be commented out.
    ...
    ofs1 << "Time" << "," << "Rep" << "," << "PopSizeF" << "," << "PopSizeM" << "," << "PopSize" << ","
        ...
        << "avg_survivalprob_true" << "," << "avg_survivalprob_rando" << "," << "avg_Res";
    ofs1 << '\n';

    //This outputs data on individuals. If you want data on population averages or the result of matings instead, then this can be commented out.
    /*std::ofstream ofs2("Filename.csv");  //Edit this to be the desired filename
    ofs2 << "Time" << "," << "Rep" << "," << "PopSizeF" << "," << "PopSizeM" << "," << "PopSize" << "," << "Sex" << ","
        << "p" << "," << "chi" << "," << "tau" << "," << "t" << "," << "gamma" << ","
        << "surviving_offspring_nr" << "," << "dead_offspring_nr" << ","
        << "Res" << "," << "NrRejections" << "," << "NrMatings";
    ofs2 << '\n';*/

    //This outputs data on the result of matings. If you want data on individuals or on population averages instead, then this can be commented out.
    /*std::ofstream ofs3("Filename.csv"); //Edit this to be the desired filename
    ofs3 << "Time" << "," << "Rep" << ","
        << "p_mother" << "," << "chi_mother" << ","
        << "tau_father" << "," << "t_father" << "," << "gamma_father" << "," << "Res_father" << ","
        << "survivalProbTrue" << "," << "survivalProbRando";
    ofs3 << '\n';*/
    ...
    for (auto current_seed = parameters.start_seed; current_seed < parameters.end_seed; ++current_seed) {
        ...
        for (int time = 0; time < parameters.max_time; time++) {
            ...
            new_males.clear();
            new_females.clear();
            double avg_survivalprob_true = 0;
            double avg_survivalprob_random = 0;
            double avg_survivalprob_true_counter = 0;
            double avg_survivalprob_random_counter = 0;
            ...
            while ((new_females.size() + new_males.size()) < parameters.max_pop_size) {
                ...
                find_mate(mother_id, father_id, has_mated, parameters, pop_females, pop_males, rng, ofs3,
                    avg_survivalprob_random, avg_survivalprob_random_counter, time);

                if (has_mated) {
                    reproduce(mother_id, father_id, parameters, rng,
                        pop_females, pop_males, new_females, new_males,
                        avg_survivalprob_true, avg_survivalprob_true_counter); 
                }
            ...
            // save data to file:
            //This outputs population averages. If you want data on individuals or the result of matings instead, then this can be commented out.
            if (time % parameters.save_interval == 0) {
                ...
                avg_survivalprob_true = avg_survivalprob_true / avg_survivalprob_true_counter;
                avg_survivalprob_random = avg_survivalprob_random / avg_survivalprob_random_counter;
                ...
                ofs1 << time << "," << current_seed << "," << pop_females.size() << "," << pop_males.size() << ","
                    ...
                    << avg_survivalprob_true << "," << avg_survivalprob_random << "," 
                    << avg_Res
                    << '\n';
            }

            //This outputs individual data. If you want data on individuals or the result of matings instead, then this can be commented out.
            /*if (time % parameters.save_interval == 0 && time != 0) {

                for (const auto& i : pop_females) {
                    ofs2 << time << "," << current_seed << "," << pop_females.size() << "," << pop_males.size() << ","
                        << pop_females.size() + pop_males.size() << ","
                        << i.S << "," << i.p << "," << i.chi << ","
                        << i.tau << "," << 'NA' << "," << 'NA' << ","
                        << i.live_offspring_count << "," << i.dead_offspring_count << ","
                        << i.Res << "," << i.NrRejections << "," << i.NrMatings
                        << '\n';
                }
    ...
    ofs1.close(); //average data
    //ofs2.close(); //individual data
    //ofs3.close(); //mating data
    return 0;
}
