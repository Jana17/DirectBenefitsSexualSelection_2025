...
struct Param {
    ...
    const size_t end_seed = 100;
    
    const double initial_tau = 0.0;
    double initial_p = 0.0;
    const double initial_chi = 10;
...
struct Individual {
    ...
    Individual(const Param& parameters,
        ...
        tau_chromo1 = parameters.initial_tau;
        tau_chromo2 = parameters.initial_tau;
        tau = (tau_chromo1 + tau_chromo2) / 2;
        p_chromo1 = parameters.initial_p;
        p_chromo2 = parameters.initial_p;
        p = (p_chromo1 + p_chromo2) / 2;
    ...
    Individual(const Individual& parent1,
        ...
        p_chromo1 = rnd.flip_coin() ? parent1.p_chromo1 : parent1.p_chromo2;
        p_chromo2 = rnd.flip_coin() ? parent2.p_chromo1 : parent2.p_chromo2;

        tau_chromo1 = rnd.flip_coin() ? parent1.tau_chromo1 : parent1.tau_chromo2;
        tau_chromo2 = rnd.flip_coin() ? parent2.tau_chromo1 : parent2.tau_chromo2;

        p_chromo1 = rnd.mutate_unbounded_trait_normalDist(p_chromo1, P.mut_stdev_a); 
        p_chromo2 = rnd.mutate_unbounded_trait_normalDist(p_chromo2, P.mut_stdev_a); 

        tau_chromo1 = rnd.mutate_bounded_trait_normalDist(tau_chromo1, P.mut_stdev_a, 0, 1);
        tau_chromo2 = rnd.mutate_bounded_trait_normalDist(tau_chromo2, P.mut_stdev_a, 0, 1);
...
int main() {
    ...
    ofs1 << "Time" << "," << "Rep" << "," << "PopSizeF" << "," << "PopSizeM" << "," << "PopSize" << ","
        ...
        << "avg_Res" << ","
        << "tau_chromo1" << "," << "tau_chromo2" << "," << "p_chromo1" << "," << "p_chromo2";
    ofs1 << '\n';
    ...
    for (auto current_seed = parameters.start_seed; current_seed < parameters.end_seed; ++current_seed) {
        ...
        for (int time = 0; time < parameters.max_time; time++) {
            ...
            while ((new_females.size() + new_males.size()) < parameters.max_pop_size) {
                ...
                find_mate(mother_id, father_id, has_mated, parameters, 
                    pop_females, pop_males, rng);
            ...
            if (time % parameters.save_interval == 0) {
                double avg_tau = 0;
                double avg_tau_chromo1 = 0;
                double avg_tau_chromo2 = 0;
                double avg_t = 0;
                double avg_p = 0;
                double avg_p_chromo1 = 0;
                double avg_p_chromo2 = 0;
                double avg_chi = 0;
                double avg_gamma = 0;
                ...
                for (const auto& i : pop_females) {
                    avg_tau += i.tau;
                    avg_tau_chromo1 += i.tau_chromo1;
                    avg_tau_chromo2 += i.tau_chromo2;
                    avg_p += i.p;
                    avg_p_chromo1 += i.p_chromo1;
                    avg_p_chromo2 += i.p_chromo2;
                    avg_chi += i.chi;
                ...
                avg_tau_chromo1 *= popsize_mult;
                avg_tau_chromo2 *= popsize_mult;
                avg_p *= popsize_mult;
                avg_p_chromo1 *= popsize_mult;
                avg_p_chromo2 *= popsize_mult;
                ...
                ofs1 << time << "," << current_seed << "," << pop_females.size() << "," << pop_males.size() << ","
                    ...
                    << avg_tau << "," << avg_p << "," << avg_chi << "," << avg_t << "," << avg_gamma << ","
                    << avg_surviving_off_f << "," << avg_dead_off_f << "," << avg_Res << ","
                    << avg_tau_chromo1 << "," << avg_tau_chromo2 << "," << avg_p_chromo1 << "," << avg_p_chromo2
                    << '\n';
            }
